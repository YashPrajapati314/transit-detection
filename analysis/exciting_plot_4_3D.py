...
def plot_before_transit_vs_during_transit_after_transit(df: pd.DataFrame, colour_map: list[str], xlabel='Before Transit Average Clustering Coefficient', ylabel='After Transit Average Clustering Coefficient', zlabel='During Transit Average Clustering Coefficient', title='', plot: bool = True):
    avg_clust_coeff_before_transit = list(df['avg_clust_coeff_before_transit'])
    avg_clust_coeff_after_transit = list(df['avg_clust_coeff_after_transit'])
    avg_clust_coeff_during_transit = list(df['avg_clust_coeff_during_transit'])
    ...
    ax.scatter(avg_clust_coeff_before_transit, avg_clust_coeff_after_transit, avg_clust_coeff_during_transit, c = 'r', s = 50)
...
def plot_relation_for_all_stars(grandparent_directory: Literal['precomputed_data', 'binned_precomputed_data']):
    ...
    plot_before_transit_vs_during_transit_after_transit(df=complete_df, colour_map=colour_map, title='Visibility Graph Average Clustering Coefficient Before, During, and After Transit Region')
